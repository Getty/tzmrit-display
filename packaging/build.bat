@echo off
rem Builds display-panel-setup-<version>.exe. Needs Python 3.10+ and NSIS
...
".venv\Scripts\python.exe" -c "from importlib.metadata import version; import pathlib; pathlib.Path('packaging/_version.txt').write_text(version('display-panel'))" || exit /b 1
...
"..\.venv\Scripts\pyinstaller.exe" --noconfirm --clean display-panel.spec || exit /b 1
...
dir /b "%~dp0display-panel-setup-*.exe"
exit /b 0
